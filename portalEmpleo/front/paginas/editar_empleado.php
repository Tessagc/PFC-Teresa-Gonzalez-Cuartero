...
echo "</header>";
        ...
        // obtener lista de departamentos e info del empleado
        $cod_empleado_editar = $_GET['cod_empleado'];
        $consulta2 = mysqli_query($conexion, "SELECT nombre, cod_departamento FROM departamentos");
        $consulta3 = mysqli_query($conexion, "SELECT * FROM empleados WHERE cod_empleado = $cod_empleado_editar");
        $info_empleado = mysqli_fetch_array($consulta3);

        // formulario
    echo "<main>";
        echo "<h2 class='text-center mt-5 mb-4 titulo-mediano fw-bold'>Editar información del empleado ".$info_empleado['nombre']." ".$info_empleado['apellidos']."</h2>";
        echo "<div class='container mt-5'>";
            echo "<div class='justify-content-center p-4'>";
                echo "<form action='../../back/acciones/actualizar_empleado.php' method='post' class='mx-auto w-75' enctype='multipart/form-data' name='nuevo_empleado'>
                        <input type='hidden' name='cod_empleado' value='$cod_empleado_editar'>
                        <div class='mb-3 px-5'>
                            <label for='dni' class='form-label fw-semibold fs-5 text-primary'>DNI:</label>
                            <input type='text' id='dni' name='dni_nuevo' class='form-control' value='".$info_empleado['dni']."' required>
                        </div>
                        
                        <div class='mb-3 px-5'>
                            <label for='nombre' class='form-label fw-semibold fs-5 text-primary'>Nombre:</label>
                            <input type='text' id='nombre' name='nombre_nuevo' class='form-control' value='".$info_empleado['nombre']."' required>
                        </div>
                        
                        <div class='mb-3 px-5'>
                            <label for='apellidos' class='form-label fw-semibold fs-5 text-primary'>Apellidos:</label>
                            <input type='text' id='apellidos' name='apellidos_nuevo' class='form-control' value='".$info_empleado['apellidos']."' required>
                        </div>
                        
                        <div class='mb-3 px-5'>
                            <label for='telefono_personal' class='form-label fw-semibold fs-5 text-primary'>Teléfono Personal:</label>
                            <input type='text' id='telefono_personal' name='telefono_personal_nuevo' class='form-control' minlength=9 value='".$info_empleado['telefono_personal']."' required>
                        </div>
                        
                        <div class='mb-3 px-5'>
                            <label for='gmail_contacto' class='form-label fw-semibold fs-5 text-primary'>Correo Electrónico de Contacto:</label>
                            <input type='email' id='gmail_contacto' name='gmail_contacto_nuevo' value='".$info_empleado['gmail_contacto']."' pattern='[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$' class='form-control' required>
                        </div>
                        
                        <div class='mb-3 px-5'>
                            <label for='gmail_empresarial' class='form-label fw-semibold fs-5 text-primary'>Correo Electrónico Empresarial:</label>
                            <input type='email' id='gmail_empresarial' name='gmail_empresarial_nuevo' value='".$info_empleado['gmail_empresarial']."' pattern='[a-zA-Z0-9._%+-]+@empresa\.com' class='form-control' required>
                        </div>

                        <div class='mb-3 px-5'>
                            <label for='puesto' class='form-label fw-semibold fs-5 text-primary'>Puesto</label>
                            <input type='text' id='puesto' name='puesto_nuevo' value='".$info_empleado['puesto']."' class='form-control' minlength=5 required>
                        </div>
